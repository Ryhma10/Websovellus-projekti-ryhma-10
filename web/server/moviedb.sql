...
-- group_movies: useita leffoja / user / group, ei duplia samasta tunnisteesta
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_group_movies_user_tmdb
  ON group_movies (group_id, user_id, tmdb_id) WHERE tmdb_id IS NOT NULL;
-- estä tupla (finnkino) samalle (group,user) -parille
CREATE UNIQUE INDEX IF NOT EXISTS uq_group_movies_user_finnkino
  ON group_movies (group_id, user_id, finnkino_id) WHERE finnkino_id IS NOT NULL;
